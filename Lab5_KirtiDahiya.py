...
def convert_american_dollars(american_dollars):
    ...
    return round(canadian_dollars, 2)                                                  # Round to the resule to two decimal places 
...
def back_day_from_trip(day_today, days_trip):
    day_back = (day_today + days_trip) % 7 
    return days_week[day_back]                                                               # To pass the unit tests : we need to return a string of the name of the week day so we use the day_back as the index to get the name of the week day 

assert back_day_from_trip(3, 5) == "Tuesday"                                                 # To see i can do print(back_day_from_trip(3,5)) to get the result of the function implementation 
...
def my_sum(nums):
    total = 0
    for num in nums:
        total += num                                                                          # This means : current = nums and total = total + current 
    return total
...
def average(nums):
    ...
    for num in nums: 
        current = num 
        total = total + current 
    return round(total / len(nums), 2)                                                          # The average is the total divided by the number of elements in the list, rounded to two decimal places 
...
def sum_of_squares(nums):
    total = 0                                                                                     # Total always starts from 0 because we are adding numbers to it. Total is used here because we want to return the sum of squares  
    for num in nums: 
        current = num ** 2                                                                        # Square the current number 
        total = total + current                                                                   # Update the total by adding the current squared number 
    return total 
...
def add_number(nums, k):
    result = []                                                                                   # Start with an empty list to store the result 
    for num in nums:
        current = num + k                                                                         # Add k to the current number 
        result.append(current)                                                                    # Now that list result is updates with new value after adding k to current number we are going to append this value to the result list 
...
# Function implementation
def squares(nums):
    result = []                                                                                        # Same as Exercise 5 , we create an empty list 
    for num in nums: 
        current = num ** 2                                                                             
