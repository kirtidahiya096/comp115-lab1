...
"""
def remove_duplicates_set(s):                                            # We are using set to store the seen characters and we are using a string to store the result. 
    """
    Write your docstring here.
    """
    res = ''                                                             # Start with an empty string to store the result. 
    seen_set = set()                                                     # Also start with an empty set to store the seen characters. 
    for c in s: 
        if c not in seen_set:
            res += c                                                     # if the character is not in the seen_set we add it to the result string. 
            seen_set.add(c)                                              # we also add the character to the seen_set to keep track of it. 
    return res 
...
"""
def gem_counting(stones, gems):                                            # we are using set to store the gems and we are using an interger variable to count the number of stones that are also gems. 
    """
    Write your docstring
    """
    gem_set = set(gems)                                                    # we convert the string of gems into a set to make it easier to check if a stone is a gem. 
    count = 0                                                              # we start the count at 0 and i will increment it every time we find a stone that is also a gem. 
    for s in stones:
        if s in gem_set:
            count += 1                                                     # if the stone is in the gem_set we increment the count by 1. 
    return count 
...
"""
def students_id(ids):                                                       # i am using a set to store the unique student ids and then i will return the length of the set as the number of different students who use the shuttle service. 
    """
    Write your docstring here
    """
    id_set = set(ids)                                                       # i convert the lists of ids into a set to get the uniques student ids. 
    return len(id_set)                                                      # i return the length of the set as the number of different students who use the shuttle service.
...
"""
def students_id_occurrences(ids):                                           # i am using a dictionary to store the student ids as keys and the number of occurences as values. i will loop throught the list and update. 
    """
    Write your docstring
    """
    ...
    for id in ids:
        if id in id_dict:
            id_dict[id] += 1                                                # if the id is already in the dictionary we increment the value by 1. 
        else:
            id_dict[id] = 1                                                 # if the id is not in the dictionary we add it to the dictionary with a value of 1. 
    return id_dict 
...
def word_frequency(paragraph):                                              # we are using a dictionary to store the words as keys and the number of occurences as values. first loop throught and then update the dictionary. 
    """
    Write your docstring
    """
    import re                                                               # we import the regular expression module to use the finall function to find all the words in paragraph. 
    word_list = re.findall(r'\b\w+\b', paragraph)                           # we use the find all function to find all the words in paragraph and store them in a list 
    word_dict = {}
